...
const cardVariants = cva(
  "flex justify-center items-center text-center transition-colors",
  {
    variants: {
      style: {
        float: "shadow-sm rounded-lg bg-white",
...
const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ className, style, padding, variant, ...props }, ref) => (
    <div
      ref={ref}
...
>(({ className, ...props }, ref) => (
  <div 
    ref={ref} 
    className={cn("flex-1 text-[#005A97] text-center font-bold text-sm leading-4", className)} 
    {...props} 
  />
));
